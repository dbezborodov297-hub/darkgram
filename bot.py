...
def load_duels():
    return load_json(DUELS_FILE, {})

def save_duels(d):
    save_json(DUELS_FILE, d)

def load_boss():
    return load_json(BOSS_FILE, {})

def save_boss(d):
    save_json(BOSS_FILE, d)
...
def send_top(cid):
    stats = load_stats()
    if not stats:
        bot.send_message(cid, "🏆 Пока пусто.")
        return
...
def cmd_duel(m):
    if m.chat.type == 'private':
        bot.send_message(m.chat.id, "⚔️ Только в группах.")
        return
    if not m.reply_to_message:
        bot.send_message(m.chat.id, "⚔️ Ответь на сообщение игрока и напиши /duel")
        return
    t = m.reply_to_message.from_user
    if t.id == m.from_user.id:
        bot.send_message(m.chat.id, "❌ Себе нельзя.")
        return
    if t.is_bot:
        bot.send_message(m.chat.id, "❌ С ботом нельзя.")
        return
    ex = get_duel(m.chat.id)
    if ex and ex.get('status') in ('pending', 'active'):
        bot.send_message(m.chat.id, "⚔️ Уже идёт дуэль.")
        return
    now = int(time.time())
    duel = {
        'chat_id': m.chat.id,
        'p1_id': str(m.from_user.id),
        'p1_name': m.from_user.first_name,
        'p2_id': str(t.id),
        'p2_name': t.first_name,
        'p1_hp': DUEL_HP,
        'p2_hp': DUEL_HP,
        'p1_aim': False,
        'p2_aim': False,
...
def cmd_boss(m):
    if m.chat.type == 'private':
        bot.send_message(m.chat.id, "🐉 Боссы только в группах.")
        return
    ...
    if cmd == 'boss':
        ...
        return
    try:
        bn = int(cmd.replace('boss', ''))
    except:
        bn = None
    if not bn or bn not in BOSSES:
        bot.send_message(m.chat.id, "❌ /boss1 ... /boss7")
        return
    b = BOSSES[bn]
    g = get_boss(m.chat.id)
    if g and g.get('status') not in ('finished',):
        bot.send_message(m.chat.id, "🐉 Уже идёт бой.")
        return
    now = int(time.time())
    ...
    g = {
        ...
        'boss_num': bn,
        'boss_name': f"{b['emoji']} {b['name']}",
        'boss_hp': b['hp'],
        'boss_max_hp': b['hp'],
        'boss_diamonds': b['diamonds'],
        'players': {},
        'turn_idx': 0,
        'status': 'lobby',
    ...
    g['players'][str(m.from_user.id)] = {
        ...
        'hp': p.get('max_hp', PLAYER_MIN_HP),
        'max_hp': p.get('max_hp', PLAYER_MIN_HP),
        'alive': True,
        'aim': False,
        'dmg_done': 0,
        'order': 0
...
def boss_lobby_tick(cid):
    ...
    if not g or g.get('status') != 'lobby':
        return
    if (g.get('deadline', 0) - int(time.time())) <= 0:
        return
...
def boss_lobby_timeout(cid):
    ...
    if len(g['players']) >= BOSS_MIN_PLAYERS:
        ...
        try:
            bot.send_message(cid, "⏱ Мало игроков. Отмена.", parse_mode='HTML')
        except:
            pass
        del_boss(cid)
...
def boss_lobby_cb(call):
    ...
    if call.data == 'b_join':
        ...
        save_boss_g(cid, g)
        sedit(cid, mid, boss_lobby_text(g), boss_lobby_kb(is_admin))
    ...
    if call.data == 'b_leave':
        ...
        if was_host and g['players']:
            new_host = sorted(g['players'].keys(), key=lambda u: g['players'][u].get('order', 0))[0]
            g['host_id'] = new_host
        elif not g['players']:
            del_boss(cid)
            sedit(cid, mid, "❌ Лобби закрыто.")
            bot.answer_callback_query(call.id, "🚪")
            return
        save_boss_g(cid, g)
        sedit(cid, mid, boss_lobby_text(g), boss_lobby_kb(is_admin))
...
def start_boss_fight(cid):
    ...
    g['status'] = 'fight'
    g['turn_idx'] = 0
    g['last_msg'] = '⚔️ Бой начался!'
    for u in g['players']:
        g['players'][u]['aim'] = False
    save_boss_g(cid, g)
    try:
...
def boss_fight_cb(call):
    cid = call.message.chat.id
    g = get_boss(cid)
    if not g or g['status'] != 'fight':
        bot.answer_callback_query(call.id, "❌ Не идёт")
        return
    uid = str(call.from_user.id)
    if uid not in g['players'] or not g['players'][uid]['alive']:
        bot.answer_callback_query(call.id, "💀 Не можешь")
        return
    ap = alive_sorted(g)
    if not ap:
...
def check_boss_end(cid, g):
    if g['boss_hp'] <= 0:
        ...
        sedit(cid, g.get('fight_msg_id'), msg)
        del_boss(cid)
        return True
    alive_list = [u for u, p in g['players'].items() if p['alive']]
    if not alive_list:
        g['status'] = 'finished'
        save_boss_g(cid, g)
        for u in g['players']:
            add_boss_loss(u)
        sedit(cid, g.get('fight_msg_id'), "💀 <b>ВСЕ ПОГИБЛИ!</b>")
        del_boss(cid)
        return True
    return False
...
def menu_cb(call):
    ...
    cid = call.message.chat.id
    if call.data == 'm_prof':
        send_profile(cid, uid, u.first_name, u.username)
    elif call.data == 'm_top':
        send_top(cid)
    elif call.data == 'm_shop':
        send_shop(cid, uid, u.first_name, u.username)
    elif call.data == 'm_duel':
        bot.send_message(cid, "⚔️ В группе ответь на сообщение игрока и напиши /duel")
    elif call.data == 'm_boss':
        bot.send_message(cid, "🐉 Напиши /boss1 ... /boss7")
